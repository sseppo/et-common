(() => {
  ...
  // tilastot_funktiot.ts
  var sortBy = async (parametrit) => {
    try {
      ...
      const filteredArray = flattenedArray.filter(
        (item) => Object.entries(rajaukset).every(([key, value]) => {
          const vastauksetRawArr = [...Object.values(item.answers)].flat();
          const vastauksetEka = vastauksetRawArr.find((answerRaw) => {
            if (answerRaw.name === key) {
              if (key === "sarja" || key === "sukupuoli") {
                if (Array.isArray(answerRaw.answer)) {
                  return answerRaw.answer.some((ans) => ans === value);
                } else {
                  return false;
                }
              } else {
  ...
  var orderBy = async (orderParams) => {
    try {
      const { filteredObjArray, orderBy: orderBy2, orderDirection } = orderParams;
      const sortedArray = filteredObjArray.sort((a, b) => {
        ...
        const aAnswer = aAnswers.find((answer) => answer.name === orderBy2);
        const bAnswer = bAnswers.find((answer) => answer.name === orderBy2);
        const aValue = aAnswers.find((answer) => answer.name === orderBy2)?.answer;
        const bValue = bAnswers.find((answer) => answer.name === orderBy2)?.answer;
  ...
  var getAnswerOf = async (answerParams) => {
    try {
      const { answers, answerOf } = answerParams;
      const vastaukset = [...Object.values(answers)].flat();
      const vastauksetEka = vastaukset.find((answerRaw) => {
  ...
  var sortByBestOf = async (bestOfParams) => {
    try {
      ...
      for (const [key, value] of Object.entries(tuloksetObjArray)) {
        ...
        console.log("filteredArr ennen lis\xE4yksi\xE4", filteredArr);
        console.log("filterByValue", filterByValue);
        console.log("bestOfvalue", bestOfvalue);
        if (!filterByValue && !bestOfvalue) {
          console.log("ei l\xF6ytynyt tulsota vaadittuja arvoja, ei lis\xE4t\xE4");
          continue;
        }
        const existingEntry = filteredArr.find((entry) => entry.filteri === filterByValue);
        console.log("existingEntry", existingEntry);
        if (existingEntry === void 0) {
          console.log("ei l\xF6ytynyt, lis\xE4t\xE4\xE4n uusi");
          if (filterByValue && bestOfvalue) {
            filteredArr.push({
          ...
          console.log("l\xF6ydettiin, p\xE4ivitet\xE4\xE4n tulos");
        ...
        console.log("filteredArr tsekkausten j\xE4lkeen ", filteredArr);
      }
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    const getFormInfo = async (formId) => {
      try {
        ...
        const response = await axios_default(axiosOptions);
        console.log("Form info response", response);
        if (response.status !== 200) {
          console.error("Error fetching tilastot data:", response.statusText);
          return;
        }
        const formInfoData = response.data.data.content;
        numberOfSubmissions = Number(formInfoData.count);
      } catch (error) {
        console.error("Error fetching tilastot data:", error);
      }
    };
    const getTilastot = async ({ offset, limit, filter: filter2, orderby }) => {
      const combinedTulokset = [];
      try {
        const urli = "http://localhost:5000/jotform/tilastot?hakuParametrit=" + JSON.stringify(hakuParametrit);
        console.log("kysely l\xE4htee, ", urli);
        const axiosOptions = {
          method: "GET",
          url: urli,
          headers: {
        ...
        const tilastotData = response.data.data;
        console.log("Tilastot data:", tilastotData);
        const tuloksetRawObj2 = tilastotData.content;
        console.log("tuloksetRawObj", tuloksetRawObj2);
        const tulolsetRawArr = [...Object.values(tuloksetRawObj2)].flat();
        console.log("tuloksetRawArray", tulolsetRawArr);
        tuloksetRArr = tuloksetRArr.concat(tulolsetRawArr);
        console.log("tuloksetRArr t\xE4ll\xE4 kiekalla", tuloksetRArr);
    ...
    const sarjat = async () => {
      ...
      if (tuloksetRArr && tuloksetRArr.length > 0) {
        const sarja_Opiskelijat_RodeoHarka_1 = await sortBy({
          ...
          rajaukset: {
            sarja: "Opiskelija",
            laji: "Rodeoh\xE4rk\xE4 - normaali",
            montakoOli: "1 - yksil\xF6suoritus"
          }
        });
        ...
        if (sarja_Opiskelijat_RodeoHarka_1) {
          ...
          if (ordederedSarja_Opiskelijat_RodeoHarka_1) {
            ...
            for (let i = 0; i < lista.length; i++) {
              ...
              if (sijoitus !== void 0 && nimi != void 0) {
          ...
          if (sarja_Oppintolinja_RodeoHarka_1) {
            ...
            for (let i = 0; i < lista.length; i++) {
              ...
              const sijoitus = "<h6>" + (i + 1) + ". Sija, aika " + tulosSekunteina + " s</h6>";
              const nimi = "<p>" + opintiolinja + "<BR /><span class='pienifontti'>" + tekija + "</span></p>";
              if (sijoitus !== void 0 && nimi != void 0) {
                const tuloste = sijoitus + nimi;
                contentToAdd += tuloste;
              }
          ...
          if (sarja_Oppilaitos_RodeoHarka_1) {
            ...
            for (let i = 0; i < lista.length; i++) {
              ...
              const sijoitus = "<h6>" + (i + 1) + ". Sija, aika " + tulosSekunteina + " s</h6>";
              const nimi = "<p>" + oppilaitos + "<BR /><span class='tilastot-pienifontti'>" + tekija + "</span></p>";
              if (sijoitus !== void 0 && nimi != void 0) {
                const tuloste = sijoitus + nimi;
                contentToAdd += tuloste;
              }
